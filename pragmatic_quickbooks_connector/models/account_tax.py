...
class AccountTax(models.Model):
    ...
    def create_account_tax(self, data):
        ...
        _logger.info("Resposne Data  : {}".format(res))
        tax_obj = False
            ...
            if 'Active' in tax and tax.get('Active'):
                if 'Taxable' in tax and tax.get('Taxable'):
                    ...
                    if 'TaxGroup' in tax and tax.get('TaxGroup'):
                        ...
                        if tax.get('PurchaseTaxRateList').get('TaxRateDetail', []):
                            for tax_rate in tax.get('PurchaseTaxRateList').get('TaxRateDetail', []):
                                purchase_tax_rate_ids.append(
                                    self.create_tax_rate(tax_rate, type_tax_use='purchase').id)
                                #                             self._cr.commit()
                            vals.update({
                        ...
                        if 'SalesTaxRateList' in tax and tax.get('SalesTaxRateList').get('TaxRateDetail', []):
                            for tax_rate in tax.get('SalesTaxRateList').get('TaxRateDetail', []):
                                sale_tax_rate_ids.append(
                                    self.create_tax_rate(tax_rate, type_tax_use='sale').id)
                                #                             self._cr.commit()
                            vals.update({
                                'type_tax_use': 'sale',
                    ...
                    raise UserError(
                        "Could not get appropriate response from quickbooks.")
        return tax_obj
        ...
        if data.status_code == 200:
            ...
            if 'TaxReturnLineRef' in res.get('TaxRate'):
                account = self.env['account.account'].search(
                    [('qbo_id', '=', res.get('TaxRate').get('TaxReturnLineRef').get('value'))], limit=1)
                ...
                if not account:
                    ...
                    data = requests.request(
                        'GET', url, headers=url_str.get('headers'))
                    if data.status_code == 200:
                        account = self.env['account.account'].create_account_account(
                            data, company.last_acc_imported_id)
            ...
            _logger.info('Tax Vals to Create : {}'.format(vals))
            tax_rate_obj = self.search([('name', '=', '{} %'.format(res.get('TaxRate').get('Name', ''))),
                                        ('qbo_tax_rate_id', '=',
                                         res.get('TaxRate').get('Id')),
                                        ('qbo_tax_id', '=', False)], limit=1)
            if not tax_rate_obj:
                tax_rate_obj = self.create(vals)
            else:
                # print('\n\nVals : ',vals)
                tax_rate_obj.write(vals)

            self.env.cr.commit()
            _logger.info(
                _("Account tax created sucessfully! Tax Id: %s" % (tax_rate_obj.id)))
                ...
                if child_tax.qbo_tax_rate_id:
                    ...
                    if not child_tax.tax_agency_id:
                        raise ValidationError(
                            _("Please select tax agency for %s" % (child_tax.name)))
