        ...
        if vals_list.get('payment_type'):
            if vals_list.get('payment_type') == 'outbound':
                if vals_list.get('partner_type') and vals_list.get('partner_id'):
                    vals_list.update({'destination_account_id': self.env['res.partner'].search(
                        [('id', '=', vals_list.get('partner_id'))],
                                ...
                                if 'LinkedTxn' in inv_rec and inv_rec.get('LinkedTxn'):
                                    if inv_rec.get('LinkedTxn') and (
                                        ...
                                        if not invoice:
                                            ...
                                            payment_method = None
                                            journal_payment_methods = None
                                            journal = self.env['account.journal'].browse(
                                                int(dup_val['journal_id']))
                                            if is_customer:
                                                dup_val.update(
                                                    {'partner_type': 'customer'})
                                                dup_val.update(
                                            ...
                                            if payment_method:
                                                journal_payment_method = journal_payment_methods.filtered(
                                                    lambda r: r.name == 'Manual')
                                                dup_val.update({
                                                    'payment_method_line_id': journal_payment_method.id
                                                })
        ...
        if payment.payment_type == 'inbound' and payment.partner_type == 'customer':
            ...
            if linked_vendor_payment_bill:
                ...
                if payment.qbo_paytype == 'check':
                    ...
                    raise ValidationError(
                        _("Please select QBO Paytype in order to export the payment."))
            else:
                _logger.info("The Payment is not linked to any vendor bill")
                raise ValidationError(
                    _("The Payment is not linked to any vendor bill.Hence,cannot be exported to QBO"))
        else:
            _logger.info(
