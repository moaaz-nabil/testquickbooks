...
class AccountAccount(models.Model):
    ...
    def get_account_ref(self, qbo_account_id):
        ...
        if not account:
            ...
            if data.status_code == 200:
                ret_id, account = self.create_account_account(data, company.last_acc_imported_id)
        return account.id
    ...
    @api.model
    def create_account_account(self, data, last_id):
...
#                 pass
            ...
            if not acc:
                ...
                _logger.info(
                    _("Account created sucessfully! Account Id: %s" % (acc.id)))
            ...
            return_id = int(account.get('Id'))
        _logger.info(_("Account updated sucessfully! Account Id: %s %s" % (acc, return_id)))
        return return_id, acc
    ...
    def export_to_qbo_main(self, accounts):
        for account in accounts:
            ...
            if account.qbo_acc_subtype:
                ...
                raise ValidationError(
                    _("QBO subtype is required for account : %s " % account.name))
    ...
    def update_account_to_qbo(self, vals):
        ...
        if access_token:
            ...
            res = requests.request('GET', quickbook_config.url + str(realmId) + "/account/{}?minorversion=12".format(
            ...
            result = requests.request('POST', quickbook_config.url + str(realmId) + "/account?minorversion=12",
            ...
            if result.status_code == 200:
                ...
                quickbook_config.last_acc_imported_id = response.get(
                    'IntuitResponse').get('Account').get('Id')
                self._cr.commit()
