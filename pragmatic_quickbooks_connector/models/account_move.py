    ...
    def create_invoice_dict(self, cust, type):
        ...
        if res_partner:
            ...
            if 'CurrencyRef' in cust and cust.get('CurrencyRef'):
                if 'value' in cust.get('CurrencyRef') and cust.get('CurrencyRef').get('value'):
                    ...
                    if not currency:
                        raise UserError(
                            _("Please activate the currency %s") % (cust.get('CurrencyRef').get('value')))

                    dict_i['currency_id'] = currency.id

            if res_partner.customer_rank and type == 'out_invoice':
...
#                         dict_i['journal_id'] = sale.id
            ...
            if res_partner.supplier_rank and type == 'in_invoice':
    ...
    def create_invoice(self, data, type='out_invoice'):
        ...
        if data:
            ...
            if parsed_data:
                ...
                if 'QueryResponse' in parsed_data and parsed_data.get('QueryResponse') and parsed_data.get('QueryResponse').get(get_data_for):
                    for cust in parsed_data.get('QueryResponse').get(get_data_for):
                        ...
                        if not account_invoice:
                            ...
                            invoice_obj = self.env['res.partner'].search([('id', '=', dict_i.get('partner_id'))],
                                                                         limit=1)
                            invoice_line = self.odoo_create_invoice_line_dict(
                                cust, invoice_obj, type, dict_i.get('qbo_invoice_id'))
                            if invoice_line:
    ...
    def odoo_create_invoice_line_dict(self, cust, invoice_obj, type, qbo_inv_id=''):
        ...
        for i in cust.get('Line'):
            ...
            if i.get('DetailType') != 'DescriptionOnly':
                ...
                if type == 'out_invoice' or type == 'out_refund':
                    if 'TxnTaxDetail' in cust and 'TxnTaxCodeRef' in cust.get('TxnTaxDetail') and cust.get('TxnTaxDetail').get('TxnTaxCodeRef'):
                    ...
                    if 'TxnTaxDetail' in cust and 'TaxLine' in cust.get('TxnTaxDetail') and cust.get('TxnTaxDetail').get('TaxLine'):
                        _logger.info(_("TxnTaxDetailTxnTaxDetail %s" % cust['TxnTaxDetail'][
                                     'TaxLine'][0]['TaxLineDetail']['TaxRateRef']['value']))
                ...
                if 'SalesItemLineDetail' in i and i.get('SalesItemLineDetail'):
                    if 'TaxCodeRef' in i.get('SalesItemLineDetail') and i.get('SalesItemLineDetail').get('TaxCodeRef'):
                    ...
                    if 'TxnTaxDetail' in cust and cust.get('TxnTaxDetail'):
                        ...
                        if 'TaxLine' in cust.get('TxnTaxDetail') and cust.get('TxnTaxDetail').get('TaxLine'):
                            ...
                            if cust.get('TxnTaxDetail').get('TaxLine')[0].get('TaxLineDetail'):
                                tax_val = cust.get('TxnTaxDetail').get('TaxLine')[0].get(
                                    'TaxLineDetail').get('TaxRateRef').get('value')
                ...
                if 'AccountBasedExpenseLineDetail' in i and i.get('AccountBasedExpenseLineDetail'):
                    ...
                    if not res_account:
                        raise UserError('Account QBO ID ' + i.get('AccountBasedExpenseLineDetail').get(
                            'AccountRef').get('value') + ' doesnot exists in Odoo. ')
                    if res_account:
                        ...
                        # Parent Id for Product Line & Customer Account
                        # Receivable Line
                        ...
                        if i.get('AccountBasedExpenseLineDetail').get('TaxCodeRef'):
                            ...
                            if type == 'in_invoice':
                                ...
                                record = self.env['account.tax']
                                tax = record.search(
                                    [('qbo_tax_id', '=', tax_val), ('type_tax_use', '=', 'purchase')], limit=1)
                        ...
                        if type == "in_invoice":
                            if invoice_obj.property_account_payable_id:
                                dict_col[
                                    'account_id'] = invoice_obj.property_account_payable_id.id
                                dict_tol[
                                    'account_id'] = invoice_obj.property_account_payable_id.id
                        ...
                        if type == "out_invoice" or type == "out_refund":
                            if invoice_obj.property_account_receivable_id:
                                dict_col[
                                    'account_id'] = invoice_obj.property_account_receivable_id.id
                                dict_tol[
                                    'account_id'] = invoice_obj.property_account_receivable_id.id
                        # if invoice_obj.property_account_receivable_id:
                ...
                if i.get(get_data_for):
                    ...
                    if res_product:
                        ...
                        # Product Id for Product Line & Customer Account
                        # Receivable Line
                        dict_ol['product_id'] = res_product.id
                        dict_col['product_id'] = False
                        dict_tol['product_id'] = False
                        ...
                        if i.get(get_data_for).get('UnitPrice'):
                            ...
                            dict_col[
                                'price_unit'] = -(float(i.get(get_data_for).get('UnitPrice')))
                        ...
                        if 'account_id' in dict_ol and 'account_id' in dict_col:
                            ...
                            _logger.info(
                                "Account ID not found in the dictionary!")
                    else:
...
#                             dict_ol['tax_ids'] = [[6, False, []]]
                    ...
                    if i.get('AccountBasedExpenseLineDetail').get('UnitPrice'):
                        ...
                        dict_ol['credit'] = dict_ol[
                            'quantity'] * float(i.get('AccountBasedExpenseLineDetail').get('UnitPrice'))
                        dict_ol['debit'] = 0
                    ...
                    if type == 'out_invoice' or type == 'out_refund':
                        if 'TxnTaxDetail' in cust and 'TxnTaxCodeRef' in cust.get('TxnTaxDetail') and cust.get('TxnTaxDetail').get('TxnTaxCodeRef'):
                            if cust.get('TxnTaxDetail').get('TxnTaxCodeRef').get('value'):
                                tax_amount = cust.get('TxnTaxDetail').get(
                                    'TaxLine')[0].get('TaxLineDetail').get('TaxPercent')
                                dict_tol['price_unit'] = float(
                                    dict_ol['quantity'] * dict_ol['price_unit'] * float(tax_amount / 100))
...
#                             dict_ol['tax_ids'] = [[6, False, []]]
                    ...
                    if 'Description' and i.get('Description'):
                        dict_ol['name'] = i.get('Description')
                        dict_col['name'] = i.get('Description')
                        dict_tol['name'] = i.get('Description')
                    else:
        ...
        if invoice.invoice_origin:
            ...
            if linked_sales_order:
                ...
                if linked_sales_order.quickbook_id:
                    ...
                    raise ValidationError(
                        _("Sales Order : %s linked to this Invoice is not exported to Quickbooks.Please export Sales Order first to link the invoice into Quickbooks " % (invoice.invoice_origin)))
                        ...
                        if result.status_code == 200:
                            ...
                            if invoice.partner_id.customer_rank:
                                invoice.qbo_invoice_id = response.get(
                                    'IntuitResponse').get('Invoice').get('Id')
                                invoice.qbo_invoice_name = response.get(
                                    'IntuitResponse').get('Invoice').get('DocNumber')
                                self._cr.commit()
                            elif invoice.partner_id.supplier_rank:
                                invoice.qbo_invoice_id = response.get(
                                    'IntuitResponse').get('Bill').get('Id')
                                invoice.qbo_invoice_name = response.get(
                                    'IntuitResponse').get('Bill').get('DocNumber')
                                self._cr.commit()
                            _logger.info(
...
#                             raise ValidationError(_("Only posted state Vendor bill is exported to QBO."))
                ...
                if not t.qbo_invoice_id:
                    ...
                    if t.state == 'posted':
                        ...
                        if data.status_code == 200:
                            ...
                            _logger.info(
                                _("STATUS CODE : %s" % (data.status_code)))
                            ...
                            raise ValidationError(
                                _("[%s] %s %s" % (data.status_code, data.reason, data.text)))
                    else:
                        raise ValidationError(
                            _("Only Posted state Invoice is exported to QBO."))
                else:
    ...
    def prepare_qbo_journal_export_dict(self):
        ...
        narration = None
        if self.ref:
            narration = '{} - {}'.format(self.name, self.ref)
