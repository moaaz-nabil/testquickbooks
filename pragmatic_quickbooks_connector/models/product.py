...
class ProductCategory(models.Model):
    ...
    qbo_product_category_id = fields.Char(
        "QBO Category Id", copy=False, help="QuickBooks database recordset id")
    ...
    def create_product_category(self, data, parent=False):
        ...
        for category in categories:
            ...
            if 'ParentRef' in category:
                ...
                # check if not category present then create otherwise use the
                # same
                vals = {
                    'name': category.get("Name", ''),
                    'qbo_product_category_id': category.get("Id"),
                }
                # If Income or expense account present then map them with odoo
                ...
                if 'IncomeAccountRef' in category:
                    if 'value' in category.get('IncomeAccountRef'):
                        ...
                        account_id = self.env['account.account'].get_account_ref(
                            category.get('IncomeAccountRef').get('value'))
                        vals.update(
                            {'property_account_income_categ_id': account_id})
                if 'ExpenseAccountRef' in category:
                    #                     account_id = self.get_income_account_ref(category)
                    if 'value' in category.get('IncomeAccountRef'):
                        account_id = self.env['account.account'].get_account_ref(
                            category.get('ExpenseAccountRef').get('value'))
                        vals.update(
                            {'property_account_expense_categ_id': account_id})
                categ_obj = self.search(
                    [('qbo_product_category_id', '=', category.get("Id"))])
                if not categ_obj:
                    categ_obj = self.create(vals)
                    _logger.info(
                        _("Product category created sucessfully! Category Id: %s" % (categ_obj.id)))
                    self._cr.commit()
                else:
    ...
    def create_category_recursively(self, category):
        if not 'ParentRef' in category:
            ...
            url_str = company.get_import_query_url()
            url = url_str.get(
                'url') + '/item/%s' % category.get('ParentRef').get('value')
            data = requests.request('GET', url, headers=url_str.get('headers'))
            ...
            if 'IncomeAccountRef' in category:
                income_account_id = self.env['account.account'].get_account_ref(
                    category.get('IncomeAccountRef').get('value'))
            #                 vals.update({'property_account_income_categ_id': account_id})
            if 'ExpenseAccountRef' in category:
                expense_account_id = self.env['account.account'].get_account_ref(
                    category.get('ExpenseAccountRef').get('value'))
            #                 vals.update({'property_account_expense_categ_id': account_id})
...
class Product(models.Model):
    ...
    qbo_product_id = fields.Char(
        'QBO Product Id', related='product_variant_ids.qbo_product_id', help="")
    ...
    def get_asset_account_ref(self):
        ...
        if access_token:
            ...
            result = requests.request('GET',
                                      company.url + str(
                                          realmId) + "/query?query=select name,acctnum from account where Name like 'Inventory Asset'",
                                      headers=headers)
    ...
    def getSyncToken(self, item_id):
        ...
        sql_query = "select Id,SyncToken from item Where Id = '{}'".format(
            str(item_id))
    ...
    def export_product_to_qbo(self):
        ...
        for product_id in self:
                ...
                if not product_id.property_account_income_id:
                    if not product_id.categ_id.property_account_income_categ_id:
                        raise ValidationError(
                            'Please Set Income Account for Product or Product Category: {}'.format(product_id.name))
                        return False
                if not product_id.property_account_expense_id:
                    if not product_id.categ_id.property_account_expense_categ_id:
                        raise ValidationError(
                            'Please Set Expense Account for Product or Category: {}'.format(product_id.name))
                        return False
            ...
            if product_id.type == "product":
                ...
                inv_asset = self.env['account.account'].search(
                    [('name', 'like', 'Inventory Asset')], limit=1)
                vals.update({
                ...
                if product_id.qbo_product_id:
                    ...
                    if update_product:
                        ...
                        synctoken = self.getSyncToken(
                            product_id.qbo_product_id)

                        if synctoken:
                            vals.update({'Id': product_id.qbo_product_id})
                            vals.update({'SyncToken': synctoken})
                            # del vals['QtyOnHand'] Commented as API now allows
                            # to update qty on hand
                            parsed_dict = json.dumps(vals)
                            _logger.info(
                                'Parsed Dict for Update Product: {} '.format(parsed_dict))
                            result = requests.request('POST', company.url + str(
                                realmId) + "/item?operation=update&minorversion=12", headers=headers,
                                data=parsed_dict)
                    ...
                    parsed_dict = json.dumps(vals)
                    _logger.info(
                        'Parsed Dict for Create Product : {} '.format(parsed_dict))
                    result = requests.request('POST', company.url + str(realmId) + "/item?minorversion=12",
                ...
                if result:
                    if result.status_code == 200:
                        ...
                        if resp_parsed.get('Item').get('Id'):
                            ...
                            product_id.qbo_product_id = resp_parsed.get(
                                'Item').get('Id')
                            _logger.info(_("Product exported sucessfully! product template Id: %s and Name : %s" % (
                                product_id.qbo_product_id, product_id.name)))
                            self._cr.commit()
    ...
    def create_product(self, data, parent=False):
        ...
        if len(products) == 0:
            raise UserError(
                "It seems that all of the Products are already imported.")
        for product in products:
            if product.get('Type') == 'Service' or product.get('Type') == 'Inventory' or product.get(
                    'Type') == 'NonInventory':
                product_type = 'consu'
                ...
                categ_id = category.get_category_ref(
                    product.get('ParentRef').get('value')) if 'ParentRef' in product else False,
                ...
                if 'IncomeAccountRef' in product:
                    vals.update({'property_account_income_id': account.get_account_ref(
                        product.get('IncomeAccountRef').get('value'))})
                else:
                    _logger.info(
                        "Income Account Reference not found,attaching from Default QBO Configuration")
                    company = self.env['res.users'].search(
                        [('id', '=', self._uid)]).company_id
                    if company.qb_income_account:
                        vals.update(
                            {'property_account_income_id': company.qb_income_account.id})
                    else:
                ...
                if 'ExpenseAccountRef' in product:
                    vals.update({'property_account_expense_id': account.get_account_ref(
                        product.get('ExpenseAccountRef').get('value'))})
                else:
                    _logger.info(
                        "Expense Account Reference not found,attaching from Default QBO Configuration")
                    company = self.env['res.users'].search(
                        [('id', '=', self._uid)]).company_id
                    if company.qb_expense_account:
                        vals.update(
                            {'property_account_expense_id': company.qb_expense_account.id})
                    else:
                ...
                if 'SalesTaxCodeRef' in product:
                    tax_id = tax.get_account_tax_ref(product.get('SalesTaxCodeRef').get('value'),
                                                     product.get(
                                                         'SalesTaxCodeRef').get('name'),
                                                     type_tax_use="sale")
                    if tax_id:
                        vals.update({'taxes_id': [(6, 0, [tax_id])]})
                if 'PurchaseTaxCodeRef' in product:
                    tax_id = tax.get_account_tax_ref(product.get('PurchaseTaxCodeRef').get('value'),
                                                     product.get(
                                                         'PurchaseTaxCodeRef').get('name'),
                                                     type_tax_use="purchase")
                    if tax_id:
                        vals.update({'supplier_taxes_id': [(6, 0, [tax_id])]})

                product_product = self.env['product.product']
                if 'Sku' in product and product.get('Sku'):
                    prod_obj = product_product.search(
                        ['|', ('default_code', '=', product.get('Sku')), ('qbo_product_id', '=', product.get("Id"))])
                else:
                    prod_obj = product_product.search(
                        [('qbo_product_id', '=', product.get("Id"))])

                if len(prod_obj) > 1:
                    raise ValidationError(_("Found multiple with internal reference %s, expected singleton" % (
                        str([p.name for p in prod_obj]))))

                if not prod_obj:
                    created = prod_obj = prod_obj.create(vals)
                    if created:
                        _logger.info(
                            _("Product created sucessfully! product template Id: %s" % (prod_obj.id)))
                else:
                    ...
                    if update_product:
                        ...
                        if updated:
                            _logger.info(
                                _("Product updated sucessfully! product template Id: %s" % (prod_obj.id)))
                self.env.cr.commit()
...
class ProductProduct(models.Model):
    ...
    is_discount_product = fields.Boolean(string="Is Discount Product", related='product_tmpl_id.is_discount_product',
                                         copy=False)
    ...
    def check_product_by_sku_in_xero(self, product_id):
        ...
        if access_token:
            ...
            headers['Content-Type'] = 'application/json'
            headers['accept'] = 'application/json'
            query = "select * from item where sku='" + \
                str(product_id.default_code) + "'"
            url = company.url + \
                str(realmId) + "/query?query=" + query + "&minorversion=12"
                    ...
                    if resp_parsed:
                        if resp_parsed.get('Item')[0].get('Id'):
                            ...
                            product_id.qbo_product_id = resp_parsed.get(
                                'Item')[0].get('Id')
                            _logger.info(
                                _("Product Id Updated successfully! product template Id: %s" % (
                                    product_id.qbo_product_id)))
                            self._cr.commit()
                            return True
    ...
    def export_product_to_qbo(self):
        ...
        for product_id in self:
            ...
            if not product_id.qbo_product_id and product_id.default_code:
                ...
                if ret:
                    _logger.info(
                        _("Product exported sucessfully! product product Id: %s and Name : %s" % (
                            product_id.qbo_product_id, product_id.name)))
                    break
                    # return True
                else:
                    _logger.info(
                        'Product Not Found In qbo by sku id : {}'.format(ret))
            ...
            if product_id.type == "product":
                ...
                if not product_id.categ_id.property_stock_valuation_account_id:
                    raise UserError(
                        _("Stock valuation account is missing for %s %s") % (product_id.default_code, product_id.name))
            ...
            if access_token:
                ...
                if product_id.qbo_product_id:
                    ...
                    if update_product:
                        vals.update({'sparse': True})
                        synctoken = self.product_tmpl_id.getSyncToken(
                            product_id.qbo_product_id)
                        if 'Name' in vals:
                            vals.pop('Name')
                        if synctoken:
