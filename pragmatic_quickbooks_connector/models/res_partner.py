...
class ResPartner(models.Model):
    ...
    x_quickbooks_exported = fields.Boolean(
        "Exported to Quickbooks ? ", default=False)
    ...
    def get_parent_customer_ref(self, qbo_parent_id):
        ...
        partner = self.search(
            [('qbo_customer_id', '=', qbo_parent_id)], limit=1)
        if not partner:
    ...
    def create_partner(self, data, is_customer=False, is_vendor=False):
        ...
        for partner in partners:
            vals = self._prepare_partner_dict(
                partner, is_customer=is_customer, is_vendor=is_vendor)
            # print("\n\n\npartner  : ", partner, '\n\n\n\n')
            ...
            if not brw_partner:
                ...
                if not brw_partner:
                    ...
                    if 'BillAddr' in partner and partner.get('BillAddr'):
                        ...
                        _logger.info(
                            "BILL ADDRESS VALS IS ---> {} and created id : {}".format(address_vals,
                                                                                      brw_partner.child_ids))
        ...
        _logger.info("\n\nBrowsing partner************ {}".format(brw_partner))
        return brw_partner
                    ...
                    if response.get('Fault').get('Error'):
                        for message in response.get('Fault').get('Error'):
                            ...
                            if message.get('code') == '6240':
                                ...
                                error = message.get('Detail')
                                _logger.warning(_("Error From Qbo for exporting Vendor : %s" % error))
