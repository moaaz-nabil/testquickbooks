        ...
        for line in self.order_line:
            ...
            if line.tax_id.id:
                ...
                if line.tax_id.qbo_tax_id:
                    ...
                    exported = self.env[
                        'account.tax'].export_one_tax_at_a_time(line.tax_id)
        ...
        for sale in sales:
            ...
            if len(sales) == 1:
                if sale.state not in ['done', 'sale']:
                    _logger.info("Only Posted records exported to qbo")
                    raise UserError("Only Posted records exported to qbo")
                    ...
                    if access_token:
                        ...
                        if sale.partner_id.customer_rank:
                            ...
                            if result.status_code == 200:
                                ...
                                if sale.partner_id.customer_rank:
                                    sale.quickbook_id = response.get(
                                        'IntuitResponse').get('Estimate').get('Id')
                                    self._cr.commit()
                                _logger.info(
                                    _("%s exported successfully to QBO" % (sale.name)))
                            #                         return True
                            else:
